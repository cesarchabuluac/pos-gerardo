...
if (isset($action) && $action == 'reset')  
{
	if (!empty($stores))
	{
		...
		$errors[] = 'La lista de tiendas seleccionada está vacía!';
	}
}
if (is_cli()) {
	if (!empty($errors)) {
...
<?php if (isset($request->get['action']) && $request->get['action'] == 'done'):?>
	...
	<div class="container">
		<div class="container-inner">
			<h2 class="alert success">Reiniciar realizado exitosamente!</h2>
		</div>
		<div class="buttons">
			<a class="btn back" href="<?php echo root_url();?>/admin/dashboard.php">&larr;Volver al Dashboard</a>
		</div>
	</div>
	</div>
<?php else:?>
	...
	<div class="container">
		<div class="container-inner">
			<?php if (!empty($errors)):?>
				...
				<?php endforeach;?>				
			<?php endif;?>
			<h2 class="title">RESET DE DATOS <br><?php echo store('name');?></h2>
			<h4 class="text">Vas a restablecer tu POS moderno. Esta acción eliminará todos los datos que no puedan restaurarse nuevamente. Si aún continúa con esta acción, haga clic en 'Sí'; de lo contrario, haga clic en el botón 'No'.</h4>
		</div>
		<div class="store-container">
	    	<h4 class="title">SELECCIONAR TIENDA</h4>
	      	<div class="store-selector">
		        <div class="checkbox selector">
		          <label class="label">
		            <input type="checkbox" onclick="$('input[name*=\'store\']').prop('checked', this.checked);"> Seleccionar / Deseleccionar
		          </label>
		        </div>
		        <div class="well"> 
		...
		<div class="buttons">
			<button class="btn yes" type="submit">SI</button>
