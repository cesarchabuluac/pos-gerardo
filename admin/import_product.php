...
if (isset($request->post['submit'])) 
{
	try {
		...
		$error = 0;
		$message = '';
		$message .= '<div><span class="fa fa-fw fa-info-circle"></span> Artículo total: ' . $total_item_no . '</div>';
		if ( count($insert_status) > 0 ) {
			...
			$message .= '<p><strong>Insertar estado</strong></p>';
			$message .= '<ul>';
			$message .= '<li>Total insertado: ' . $success . '</li>';
			$message .= '<li>Error en: ' . $error . '</li>';
			$message .= '</ul>';
		}
		...
		if (count($update_status) > 0) {
			...
			$message .= '<p><strong>Estado de actualización</strong></p>';
			$message .= '<ul>';
			$message .= '<li>Total actualizado: ' . $success . '</li>';
			$message .= '<li>Sin cambios en: ' . $error . '</li>';
			$message .= '</ul>';
		}
	    ...
	    <div class="box">
	      <div class="box-body">
	        ...
	        <div class="alert alert-danger mb-0">
	          <p><span class="fa fa-fw fa-info-circle"></span> La función de importación de productos está deshabilitada en la versión demo</p>
	        </div>
	      </div>
	    </div>
	    <?php endif; ?>
						...
						<div class="box-body">
							...
							<div class="well well-small">
								<div class="text-warning">
									<div>La primera línea en el archivo .xls descargado debe permanecer como está. Por favor no cambie el orden de las columnas. Asegúrese de que el archivo (* .xls) esté codificado en UTF-8. Las imágenes deben cargarse en la carpeta de almacenamiento / productos / (o donde señaló). El sistema verificará que si existe una fila, actualice, si no existe, inserte.
									</div>
								</div>
							</div>
