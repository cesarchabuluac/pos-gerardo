...
if(!checkInternetConnection() || !checkValidationServerConnection() || !checkEnvatoServerConnection()) {
	if (is_ajax()) {
		...
		$errors['internet_connection'] = 'Necesita conexión a internet!';
	}
}
...
function purchase_code_validation() 
{
    ...
    if (empty($request->post['purchase_username'])) {
        $errors['purchase_username'] = 'Se requiere comprar nombre de usuario';
        return false;
    }

    if (empty($request->post['purchase_code'])) {
        $errors['purchase_code'] = 'Se requiere código de compra';
        return false;
    }

    $file = DIR_INCLUDE.'config/purchase.php';
    if (is_writable($config_path) === false) {
        $errors['config_error'] = 'config.php no se puede escribir!';
        return false;
    }

    if (is_writable($ecnesil_path) === false) {
        $errors['config_error'] = 'Algunos archivos no pueden escribir!';
        return false;
    }
    ...
    if (!is_object($apiCall)) {
        $errors['internet_connection'] = 'Validación fallida!';
        return false;
    }
    if($apiCall->status == 'error') {
        ...
        $errors['preparation'] = 'Problema al generar licencia!';
        return false;
    }
}
...
if (isset($request->get['action_type']) && $request->get['action_type'] == 'UPDATESYSTEM')
{
    ...
    if (mysqli_connect_errno()) {
        $json['error'] = 'Oop!, Algo salió mal. Por favor revise su entrada';
    }

    $filepath = DIR_STORAGE.".sql";

    if (!is_file($filepath)) {
        $json['error'] = 'El archivo temporal no existe';
    }   
        ...
        if ($data_available) {
            ...
            if (isset($result['contents'])) {
              foreach ($result['contents'] as $filename => $content) {
                ...
                $errors['preparation'] = 'Problema al preparar archivos! ';
                return false;
            }


            $json['success'] = 'Tablas de base de datos importadas con éxito';
        }
    }
...
function database_import() 
{
    ...
    if(!$apiCall || !is_object($apiCall)) {
        $errors['dbimport'] = 'Una respuesta inesperada del servidor de validación!';
        return false;
    }
    if ($apiCall->status == 'error') {
    ...
    if(empty($apiCall->schema)) {
        $errors['dbimport'] = 'Sql no fue encontrado!';
        return false;
    }
    ...
    if (is_writable($config_path) === false) {
        $errors['database_import'] = 'El archivo de configuración no se puede escribir';
        return false;
    } else {
        $file = $config_path;
    ...
    if ($settings['version'] == $version) {
        $errors['error'] = 'El sistema ya está actualizado!';
        return false;
    }
...
if ($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'STARTUPDATE') 
{
    ...
    if (!check_pcode()) {
        $errors['purchase_code'] = 'El código de compra no es válido.';
    }

    if (empty($request->post['host'])) {
        $errors['host'] = 'Se requiere campo de host.';
    }

    if (empty($request->post['database'])) {
        $errors['database'] = 'El campo de la base de datos es obligatorio.';
    }

    if (empty($request->post['user'])) {
        $errors['user'] = 'El campo de nombre de usuario es obligatorio.';
    }

    if (empty($request->post['port'])) {
        $errors['port'] = 'Se requiere campo de puerto.';
    }
...
if ($request->server['REQUEST_METHOD'] == 'POST') 
{
    ...
    if (empty($request->post['purchase_username'])) {
        $errors['purchase_code'] = 'El nombre de usuario de compra no es válido.';
    }
...
<html lang="en">
  <head>
    ...
    <title>Actualice Modern POS de v2.0 - v3.0</title>
        ...
        <div class="col-sm-8 col-sm-offset-2">
            <div class="panel panel-default header" style="">
                <div class="panel-heading text-center">
                    <h2>Actualice Modern POS de v2.0 a v3.0</h2>
                    <h1>PASO
                ...
                <?php if (!isset($request->get['update'])):?>
                    <div class="panel-body">
                        ...
                        <p style="color:#ff023b;">Aunque lo hemos probado varias veces, pero no podemos ofrecerle un 100% de garantía de que, después de una actualización exitosa, el sistema funcionará correctamente. Entonces, antes de proceder a la actualización. Por favor, lea / siga la lista de requisitos previos. Usted es solo la persona responsable mientras el sistema no funciona después de una actualización exitosa.</p>

                        <div class="text-center">
                        <h4 class="bg-info" style="padding:10px;"><b>NECESITA INTERNET, ASEGÚRESE DE QUE SU CONEXIÓN A INTERNET ESTÁ BIEN</b></h4>
                        <h4 class="bg-info" style="padding:10px;"><b>TOMAR COPIA DE SEGURIDAD</b></h4>
                        <h4 class="bg-info" style="padding:10px;"><b>HAGA UNA COPIA DE SEGURIDAD DE TODOS LOS ARCHIVOS</b></h4>
                        </div>

                        <div class="alert alert-warning highlight-text" style="margin-bottom:0!important;">
                            <p>*** Esta acción puede tomar varios minutos. Tenga paciencia mientras procesa esta acción y nunca cierre el navegador. De lo contrario, el sistema no funcionará correctamente. Disfruta de una taza de café mientras esperas ... :)</p>
                        </div>
                    </div>
                <?php endif;?>
            </div>
